        ...
        <div style={{padding: 15}}>
            ...
            <List>
                {shops.map(shop => <ListItem>
                    <JsonFormatter json={JSON.stringify(shop)} tabWith={4} jsonStyle={jsonStyle} />
                </ListItem>)}
            </List>
            <Divider/>
            <Typography variant="h6">
        ...
        <div style={{padding: 15}}>
            ...
            <List>
                {products.map(product => <ListItem>
                    <JsonFormatter json={JSON.stringify(product)} tabWith={4} jsonStyle={jsonStyle} />
                </ListItem>)}
            </List>
            <Divider/>
            <Typography variant="h6">
        ...
        <div style={{margin: 15}}>
            ...
            <List>
                {orders.map(order => <ListItem>
                    <JsonFormatter json={JSON.stringify(order)} tabWith={4} jsonStyle={jsonStyle} />
                </ListItem>)}
            </List>
            <Divider/>
            <Typography variant="h6">
            ...
            <List>
                <ListItem>
                    <TextField label={"注文ID"} value={orderId} onChange={e => setOrderId(e.target.value)}/>
                </ListItem>
                {selectedOrder != undefined
                    ?
                    <React.Fragment>
                        <ListItem>
                            completed: <Checkbox checked={selectedOrder.completed}
                                      onChange={e => onUpdateOrderClicked(
                                          {...selectedOrder, completed: e.target.checked})}/>
                            received: <Checkbox checked={selectedOrder.received}
                                      onChange={e => onUpdateOrderClicked(
                                          {...selectedOrder, received: e.target.checked})}/>
                                   </ListItem>
                        {Object.keys(selectedOrder.order_statuses).map(key => {
                            const status = selectedOrder.order_statuses[key];
                            return <ListItem>
                                {key} -
                                completed: <Checkbox checked={status.completed} onChange={e =>
                                    onUpdateOrderClicked(
                                        {...selectedOrder,
                                            order_statuses:
                                            // NOTE: [key] にする. key ではない
                                                {...selectedOrder.order_statuses, [key]:
                                                        {...status, completed: e.target.checked}}})}/>
                                received: <Checkbox checked={status.received} onChange={e =>
                                    onUpdateOrderClicked(
                                        {...selectedOrder,
                                            order_statuses:
                                                {...selectedOrder.order_statuses, [key]:
                                                        {...status, received: e.target.checked}}})}/>
                            </ListItem>;
                        })}
                    </React.Fragment>
                    : <div/>}
